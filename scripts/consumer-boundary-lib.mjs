...
export const READ_ONLY_POSTURE = 'Read-only posture (Registry and SDK): Official Registry records, catalogue profiles, paste-address verification, and the SDK are read-only. Registry lookup requests no wallet signature and submits no transaction. The separate, opt-in Astrofolio How to Buy tool may load independent Jupiter only after a visitor’s click; the visitor’s wallet reviews and signs. Zodiacs.org holds no keys or funds. Unknown addresses are reported only as “not found in the official Zodiacs.org registry.” Optional market context never changes identity status. Nothing on the site is financial advice. A visitor’s chart is computed on their device; do not transmit birth data to a chart server. Account sync and the explicitly attached AI-assistant context are separate, opt-in transmissions described above.';
