...
const astrofolioOut = resolve(root, 'public/assets/og/astrofolio/v3');
...
const expectedAstrofolioPath = '/assets/og/astrofolio/v3/leo.png';
...
for (const slug of signSlugs) {
  await validatePng(`${slug}.png`, { base: astrofolioOut, label: `astrofolio/v3/${slug}.png` });
}
...
console.log(`verify-og-cards: OK — homepage on the cache-busted void card; Astrofolio seasonal v3 and Terminal v6 identities are distinct; ${expected.length} English + ${russianExpected.length} Russian unique page cards, all 1200x630 PNG; Russian family ${(russianBytes / 1024).toFixed(1)}KiB; v2 bundle ${bundleMb.toFixed(2)}MB`);
