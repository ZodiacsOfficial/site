...
export async function generateAssistantContext({ repoRoot = repo } = {}) {
  ...
  const context = [
    ...
    'Historical civil time uses the IANA/ICU history supplied by the visitor’s browser or device runtime, so historical coverage and tzdb version depend on that host. A birth from before its place adopted a legal time uses the birthplace’s own local mean time, from its longitude, with the date each place adopted a legal time taken from a pinned tzdb release. When birth time is unknown, the site uses 12:00 local civil time as a reference for body positions, omits the rising sign, angles, and houses, and flags uncertainty if the Moon changes signs during that local date.',
