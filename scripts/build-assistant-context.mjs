...
export async function generateAssistantContext({ repoRoot = repo } = {}) {
  ...
  const context = [
    ...
    'Chart calculation does not send birth fields to a chart API. Saved charts are local-first; optional account sync uploads only the charts a person chooses, including their birth details, to that person’s account. The AI assistant sends chat messages to Anthropic. “Ask why” on Today may add one visible, removable client-computed transit receipt containing the date, moving body, aspect, natal point, and orb to the question being sent. A full placements-only chart summary is sent only after the person explicitly chooses “Attach my chart”; the assistant does not automatically attach the saved name, birth date, time, place, or coordinates.',
