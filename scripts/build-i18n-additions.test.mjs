...
describe('additive locale handoff manifest', () => {
  it('covers the exact 570-key additive source set without duplicates', () => {
  ...
  it('renders all per-key fields and keeps the checked-in manifest byte-current', async () => {
    ...
    expect(rendered).toContain('TODO(i18n): translation handoff for 570 additive keys');
    ...
    expect(rendered.match(/^  - EN default:/gm)).toHaveLength(570);
    expect(rendered.match(/^  - Usage:/gm)).toHaveLength(570);
    expect(rendered.match(/^  - Pending locales:/gm)).toHaveLength(570);
