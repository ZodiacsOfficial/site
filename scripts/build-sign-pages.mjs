...
${JSON.stringify(jsonLd(m), null, 2)}
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .hdr__mark:hover, .hdr__mark:hover .dim { color: var(--gold-bright); }
    ...
    /* ── Mobile menu — burger + overlay (≤700px) ──
       The header chips crowd a phone; below 700px they fold into a burger
       overlay that also surfaces the wider wing (Registry · The Twelve ·
       Verify · Thesis · Archive · SDK). Parity with the hub, museum register. */
    .hdr__burger {
      display: none;
      width: 34px; height: 34px; margin-left: 2px; padding: 0;
      gap: 4px; place-content: center; place-items: center;
      border: 1px solid var(--hair-2); border-radius: 999px;
      background: rgba(198,204,218,0.05); cursor: pointer;
    }
    .hdr__burger-line {
    ...
    .hdr__burger[aria-expanded='true'] .hdr__burger-line:first-child { transform: translateY(5.5px) rotate(45deg); }
    .hdr__burger[aria-expanded='true'] .hdr__burger-line:nth-child(2) { opacity: 0; transform: scaleX(0.4); }
    .hdr__burger[aria-expanded='true'] .hdr__burger-line:last-child { transform: translateY(-5.5px) rotate(-45deg); }
    .hdr__menu {
      ...
      padding: calc(92px + env(safe-area-inset-top)) clamp(24px, 7vw, 40px) 40px;
      overflow-y: auto;
    }
    .hdr__menu[hidden] { display: none; }
    .hdr__menu-inner { display: flex; flex-direction: column; max-width: 520px; margin: 0 auto; }
    .hdr__menu-label {
      margin-bottom: 18px;
      font-family: var(--mono); font-size: 10px; letter-spacing: 0.24em;
    ...
    .hdr__menu-inner a {
      ...
      font-size: clamp(26px, 8vw, 34px); letter-spacing: 0.01em; line-height: 1.05;
      color: var(--ink); text-decoration: none;
      border-bottom: 1px solid var(--hair);
      transition: color 320ms var(--ease);
    }
    .hdr__menu-inner a:last-child { border-bottom: 0; }
    .hdr__menu-inner a:hover { color: var(--gold-bright); }
    .hdr__menu.is-open a, .hdr__menu.is-open .hdr__menu-label {
      animation: hdrMenuIn 520ms var(--ease) both;
      animation-delay: calc(40ms * var(--i, 0));
    }
    @keyframes hdrMenuIn { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: none; } }
    @media (max-width: 700px) {
      .hdr__nav { display: none; }
      .hdr__burger { display: inline-grid; }
    }
    @media (min-width: 701px) { .hdr__menu { display: none; } }
    @media (prefers-reduced-motion: reduce) {
      .hdr__menu.is-open a, .hdr__menu.is-open .hdr__menu-label { animation: none; }
    }
...
<body>
  ...
  <div class="hdr-wrap">
    <header class="hdr" role="banner">
      ...
      <button type="button" class="hdr__burger" data-menu-toggle aria-expanded="false" aria-controls="hdr-menu" aria-label="Open menu">
        <span class="hdr__burger-line"></span><span class="hdr__burger-line"></span><span class="hdr__burger-line"></span>
      </button>
    </header>
  </div>
  <div id="hdr-menu" class="hdr__menu" data-menu hidden>
    <nav class="hdr__menu-inner" aria-label="Registry sections">
      <span class="hdr__menu-label">The Registry</span>
      <a href="/registry/" style="--i:1">Registry</a>
      <a href="/registry/#official-twelve" style="--i:2">The Twelve</a>
      <a href="/registry/#verify" style="--i:3">Verify</a>
      <a href="/thesis/" style="--i:4">Thesis</a>
      <a href="/archive/" style="--i:5">Archive</a>
      <a href="/sdk/" style="--i:6">SDK</a>
    </nav>
  </div>
        ...
        <figure class="card" style="margin:0">
          ...
          <div class="card__inner">
            <div class="card__head">
              <span class="label">Nº ${String(m.order).padStart(2, '0')} / 12</span>
...
${CHANNELS.map((c) => `          <a href="${c.url}" rel="noopener noreferrer">${c.name}</a>`).join('\n')}
  ...
  (function () {
    ...
    // Mobile menu — burger overlay (parity with the hub / main site). Below
    // 700px the header chips fold into this; it also surfaces the wider wing.
