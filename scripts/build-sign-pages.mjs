...
function render(m) {
  ...
  const specRows = [
    ['Lot', `${p.lot} of XII`, true],
    ['Dates', p.datesDisplay, true],
    ['Element', titleCase(meta.element)],
    ['Modality', titleCase(meta.modality)],
    ['Ruler', meta.rulingPlanet],
    ['Archetype', meta.archetype],
    ['Principal star', `${p.principalStar.name} — ${p.principalStar.note}`],
    ['Babylonian record', p.babylonianRecord],
    ['Medium', 'Native SPL record, Solana · Official ERC-20 representation, Base'],
    ['Edition', 'One canonical record of twelve']
  ];
...
<head>
  ...
  <meta name="twitter:image" content="https://zodiacs.org/assets/og/${m.slug}.png" />

  <link rel="icon" type="image/svg+xml" href="data:image/svg+xml;utf8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect width='64' height='64' rx='10' fill='%2308090C'/%3E%3Ctext x='50%25' y='50%25' dominant-baseline='central' text-anchor='middle' font-family='Cormorant Garamond, Georgia, serif' font-style='italic' font-size='42' fill='%23DEC07A'%3EZ%3C/text%3E%3C/svg%3E" />
...
${JSON.stringify(jsonLd(m), null, 2)}
  ...
  <style>
    :root {
      --bg: #050609; --bg-2: #080A0F;
      --surface: #10131A; --surface-2: #161A23;
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    body {
      min-height: 100dvh;
      background:
        radial-gradient(120% 60% at 50% -10%, rgba(201,169,97,0.05), transparent 62%),
        radial-gradient(120% 80% at 50% 110%, rgba(201,169,97,0.02), transparent 62%),
        var(--bg);
    }
    ...
    .stars {
      ...
      background-image:
        radial-gradient(1px 1px at 8% 12%, rgba(232,227,212,0.7), transparent 60%),
        radial-gradient(1px 1px at 22% 38%, rgba(201,169,97,0.6), transparent 60%),
        radial-gradient(1px 1px at 71% 8%, rgba(232,227,212,0.45), transparent 60%),
        radial-gradient(1px 1px at 88% 24%, rgba(201,169,97,0.55), transparent 60%),
        radial-gradient(1px 1px at 44% 62%, rgba(232,227,212,0.4), transparent 60%),
        radial-gradient(1px 1px at 92% 71%, rgba(232,227,212,0.5), transparent 60%);
    ...
    .hdr {
      ...
      box-shadow:
        inset 0 1px 0 rgba(222,192,122,0.10), inset 0 -1px 0 rgba(0,0,0,0.4),
    ...
    .hdr__nav .chip {
      ...
      border: 1px solid var(--hair-2); background: rgba(201,169,97,0.05);
    ...
    .card {
      position: relative;
      background:
        radial-gradient(120% 60% at 50% 0%, rgba(201,169,97,0.06), transparent 55%),
        linear-gradient(180deg, #14171F 0%, #0C0E14 100%);
      border: 1px solid var(--hair-3);
      box-shadow:
        inset 0 1px 0 rgba(222,192,122,0.14), inset 0 -1px 0 rgba(0,0,0,0.5),
    ...
    .stage::after {
      ...
      background: linear-gradient(90deg, transparent 0%, rgba(201,169,97,0.55) 22%, rgba(201,169,97,0.55) 78%, transparent 100%);
      pointer-events: none; z-index: 0;
    }
    .stage img {
    ...
    .rec {
      border: 1px solid var(--hair-2);
      background: linear-gradient(180deg, rgba(201,169,97,0.03), transparent 40%), var(--surface);
    ...
    .rec__link {
      ...
      border-bottom: 1px solid rgba(201,169,97,0.32); padding-bottom: 2px;
    ...
    .copychip {
      display: inline-flex; align-items: center; gap: 8px;
      padding: 6px 10px; border: 1px solid var(--hair-2);
      background: rgba(201,169,97,0.04); color: var(--ink-2);
    ...
    .copychip.is-copied { border-color: rgba(111,174,124,0.55); color: var(--live); }
    .copychip.is-copied .copychip__icon { color: var(--live); }

    /* Market panel */
    .market {
      position: relative; border: 1px solid var(--hair);
      background: linear-gradient(180deg, rgba(201,169,97,0.025), transparent 62%), rgba(8,10,14,0.52);
    }
    .market::after {
    ...
    @media (min-width: 640px) {
      ...
      .market__cell:nth-child(3n) { border-right: 0; }
    }
    ...
    .btn {
      ...
      box-shadow: inset 0 1px 0 rgba(222,192,122,0.12), inset 0 -1px 0 rgba(0,0,0,0.5);
    ...
    .btn .arr {
      ...
      background: rgba(201,169,97,0.06); border: 1px solid var(--hair-2);
      color: var(--gold); font-size: 13px;
      transition: transform 420ms var(--ease), background 320ms var(--ease), border-color 320ms var(--ease);
    }
    .btn:hover .arr { transform: translate(3px, -1px); background: rgba(201,169,97,0.14); border-color: var(--gold); }
    .acq__note {
    ...
    .nextlot a:hover { background: rgba(201,169,97,0.05); }
    ...
    .nextlot .lt { font-family: var(--mono); font-size: 9.5px; letter-spacing: 0.22em; color: var(--gold); text-transform: uppercase; }
...
<body>
  ...
  <main class="pg" id="main">
    ...
    <div class="split">
        ...
        <figure class="card" style="margin:0">
          ...
          <div class="card__inner">
            <div class="card__head">
              <span class="label">№ ${String(m.order).padStart(2, '0')} / 12</span>
      ...
      <div>
        <section class="sec reveal" style="padding-top:8px" aria-label="Museum label">
          <div class="sec__head"><h2 class="sec__title">Museum label</h2><span class="line"></span></div>
          <div class="rows">
${specRows.map(([k, v, mono]) => `            <div class="row"><span class="k">${esc(k)}</span><span class="v${mono ? ' mono' : ''}">${esc(v)}</span></div>`).join('\n')}
          </div>
        </section>

        <section class="sec reveal" aria-label="Catalogue note">
          <div class="sec__head"><h2 class="sec__title">Catalogue note</h2><span class="line"></span></div>
          <div class="note">
...
${p.note.map((para) => `            <p>${esc(para)}</p>`).join('\n')}
    ...
    <section class="sec reveal" id="provenance" aria-label="Provenance">
      <div class="sec__head"><h2 class="sec__title">Provenance</h2><span class="line"></span></div>
      <div class="prov">
${beats.map((b) => `        <div class="prov__item">
    ...
    <section class="sec reveal" id="record" aria-label="The record">
...
${CHANNELS.map((c) => `          <a href="${c.url}" rel="noopener noreferrer">${c.name}</a>`).join('\n')}
  ...
  (function () {
    ...
    // Market context — same quiet semantics as the main page: lazy,
    // third-party, unavailable-safe.
    ...
    function setUnavailable() { stateEl.textContent = 'Market context unavailable.'; }
    ...
    function fmtDex(v) {
      if (!v) return '—';
      var s = String(v).replace(/[-_]/g, ' ').replace(/\\b\\w/g, function (c) { return c.toUpperCase(); });
    ...
    function renderMarket(pair) {
      var change = Number(pair.priceChange && pair.priceChange.h24);
      var changeClass = isFinite(change) ? (change > 0 ? ' market__v--up' : change < 0 ? ' market__v--down' : '') : '';
      var cells = [
        ['Price USD', fmtPrice(pair.priceUsd), ' market__v--mono'],
        ['24H Change', fmtPct(pair.priceChange && pair.priceChange.h24), ' market__v--mono' + changeClass],
        ['Liquidity', fmtCompact(pair.liquidity && pair.liquidity.usd), ''],
        ['Market Cap', fmtCompact(pair.marketCap), ''],
        ['Pair Created', fmtDate(pair.pairCreatedAt), ''],
        ['Source / DEX', fmtDex(pair.dexId), '']
      ];
      gridEl.innerHTML = cells.map(function (c) {
        return '<div class="market__cell"><div class="market__k">' + c[0] +
          '</div><div class="market__v' + c[2] + '">' + c[1] + '</div></div>';
      }).join('');
      stateEl.hidden = true;
      gridEl.hidden = false;
    }
    ...
    function loadMarket() {
      if (!MARKET) { setUnavailable(); return; }
      ...
      fetch(url).then(function (res) {
        ...
        if (!pairs || !pairs.length) { setUnavailable(); return; }
        renderMarket(pairs[0]);
