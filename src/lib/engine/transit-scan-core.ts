 ...
 * is injected by a thin browser or server wrapper, and every exact pass is
 * retained — including the direct/retrograde/direct sequence around a station.
 */
...
export interface TransitEphemeris {
  ...
  findLongitudeCrossings(
    ...
    to: Date,
    stepDays?: number,
  ): { at: Date; retrograde: boolean }[];
  longitudeSpeed(body: BodyName, date: Date): number;
}

export interface TransitScanner {
  scanTransitContacts(
...
function refineLongitudeExtremum(
  ...
  const referenceLon = ephemeris.bodyLongitude(body, new Date((fromMs + toMs) / 2));
  const valueAt = (time: number) => {
    const lon = ephemeris.bodyLongitude(body, new Date(time));
...
function stationBreakpoints(
  ...
  let previousSpeed = ephemeris.longitudeSpeed(body, from);

  while (previousMs < toMs) {
    const currentMs = Math.min(previousMs + stepMs, toMs);
    const currentSpeed = ephemeris.longitudeSpeed(body, new Date(currentMs));
 ...
 */
function scanTransitContactsWith(
  ephemeris: TransitEphemeris,
  natal: NatalTransitChart,
  from: Date,
  to: Date,
  ...
  for (const transitBody of bodies) {
    ...
    const stations = stationBreakpoints(ephemeris, transitBody, from, to, stepDays);
      ...
      for (const aspect of aspects) {
        for (const targetLon of aspectTargetLongitudes(point.lon, aspect)) {
          const hits = boundaries.slice(0, -1).flatMap((boundary, index) =>
            ephemeris.findLongitudeCrossings(
              transitBody,
              targetLon,
              boundary,
              boundaries[index + 1],
          ...
          for (const station of stations) {
            ...
            const stationLon = ephemeris.bodyLongitude(transitBody, station);
            const resolution = Math.max(
              angularError(
                stationLon,
                ephemeris.bodyLongitude(transitBody, new Date(stationMs - 500)),
              ),
              angularError(
                stationLon,
                ephemeris.bodyLongitude(transitBody, new Date(stationMs + 500)),
          ...
          if (angularError(targetLon, ephemeris.bodyLongitude(transitBody, from)) < 1e-8) {
...
export function createTransitScanner(ephemeris: TransitEphemeris): TransitScanner {
  return {
    scanTransitContacts: (natal, from, to, options) =>
      scanTransitContactsWith(ephemeris, natal, from, to, options),
  };
}
