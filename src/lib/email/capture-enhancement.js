...
function enhanceEmailCapture(initialContext) {
  if (initialized) return;
  initialized = true;
  const captureGenerations = /* @__PURE__ */ new WeakMap();
  const captureGeneration = (surface) => surface ? captureGenerations.get(surface) ?? 0 : 0;
  ...
  const captureSurfaces = document.querySelectorAll("[data-email-capture-shell]");
  ...
  function preselectCaptureSign(surface, sign) {
    ...
    if (!radio || !sign) {
      delete surface.dataset.personalizedSign;
      radios.forEach((input) => {
        input.checked = input.value === "";
        input.defaultChecked = input.value === "";
      });
      renderCaptureSign(surface, void 0, false);
      return;
    }
  ...
  function revealChartCapture(event, preserveManagedVisibility = false) {
    ...
    document.querySelectorAll('[data-reveal-on-chart="true"]').forEach((surface) => {
      ...
      if (!managed || !preserveManagedVisibility) surface.hidden = managed;
    });
  }
  window.addEventListener("zodiacs:chart-computed", revealChartCapture);
  function clearChartCapture() {
    document.querySelectorAll('[data-reveal-on-chart="true"]').forEach((surface) => {
      advanceCapture(surface);
      surface.hidden = true;
    });
  }
  window.addEventListener("zodiacs:chart-context-cleared", clearChartCapture);
  ...
  // A late chunk must not broadcast an old calculation to the coordinator or
  // hide a managed surface whose current state the coordinator already resolved.
  if (initialContext?.type === 'zodiacs:chart-computed') {
    revealChartCapture(new CustomEvent(initialContext.type, { detail: initialContext.detail }), true);
  } else if (initialContext?.type === 'zodiacs:chart-context-cleared') {
    clearChartCapture();
  }
}
export {
