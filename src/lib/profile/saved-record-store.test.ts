...
class MemoryAdapter implements SavedNatalAdapter {
  ...
  async read(owner: string, id: string | null, guard: () => void): Promise<unknown[]> {
    ...
    guard();
    if (this.backend.erased.has(owner)) throw new DOMException('terminal erasure', 'InvalidStateError');
...
describe('owner-bound immutable saved natal operations', () => {
  ...
  it('explicit clear removes corrupt/future owned content, preserves other owners, and fences a separate handle', async () => {
    ...
    expect((await separateHandle.store.create(polar.envelope)).ok).toBe(false);
    expect((await separateHandle.store.list()).ok).toBe(false);
    expect(backend.owner(A).size).toBe(0);
...
describe('native adapter bounded opening failures', () => {
  ...
  it('closes a late successful native open after a blocked event', async () => {
    ...
    const close = vi.fn();
    const database = { close, version: 2, objectStoreNames: { length: 2, contains: () => true } };
    ...
    expect(indexedDB.open).toHaveBeenCalledWith(SAVED_NATAL_DATABASE_NAME, 2);
  });
