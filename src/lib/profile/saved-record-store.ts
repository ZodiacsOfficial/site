...
// Unreleased v1 databases are preserved, not silently upgraded. Raising the
// version also prevents an old v1 adapter from writing around durable barriers.
const VERSION = 2;
const ERASURES = 'erasures';
const ALL_OWNERS = '*';
interface Erasure { readonly target: string; readonly status: 'pending' | 'complete' }
function parseErasure(value: unknown, target: string): Erasure | undefined {
  if (value === undefined) return undefined;
  if (value === null || typeof value !== 'object' || Object.getPrototypeOf(value) !== Object.prototype) {
    throw issue('unsupported-storage');
  }
  const fields = Object.getOwnPropertyDescriptors(value);
  if (Reflect.ownKeys(fields).length !== 2 || !fields.target || !fields.status
    || !('value' in fields.target) || !('value' in fields.status)
    || fields.target.value !== target || (target !== ALL_OWNERS && !isSavedNatalOwnerKey(target))
    || !['pending', 'complete'].includes(fields.status.value)) throw issue('unsupported-storage');
  return { target, status: fields.status.value };
}
...
export type SavedNatalFailureCode = 'access-denied' | 'stale' | 'invalid-input'
  ...
  | 'erasure-pending' | 'owner-erased' | 'storage-unavailable' | 'quota-exceeded' | 'blocked' | 'aborted' | 'full' | 'id-conflict';
...
/**
 * One adapter belongs to one handle. Production adapters must serialize the
 * erasure barrier with EVERY read/write, persist intent before deleting, and
 * retain a terminal fence after deletion. Test adapters model only stated cases.
 */
...
export class IndexedDbSavedNatalAdapter implements SavedNatalAdapter {
      ...
      request.onsuccess = () => {
        ...
        try {
          ...
          if (database.version !== VERSION || database.objectStoreNames.length !== 2
            || !database.objectStoreNames.contains(RECORDS) || !database.objectStoreNames.contains(ERASURES)) throw issue('unsupported-storage');
  ...
  private async execute<T>(
    ...
    guard: Guard,
    queue: (store: IDBObjectStore, done: (value: T) => void, fail: (error: unknown) => void, check: Guard, erasures: IDBObjectStore) => void,
    erasing = false,
  ): Promise<T> {
    guard();
    ...
    return new Promise((resolve, reject) => {
      ...
      try { transaction = database.transaction([RECORDS, ERASURES], mode); }
      ...
      try {
        check();
        if (!isSavedNatalOwnerKey(ownerKey) && !(erasing && ownerKey === ALL_OWNERS)) throw issue('invalid-input');
        ...
        const erasures = transaction.objectStore(ERASURES);
        if (erasures.autoIncrement || erasures.keyPath !== 'target' || erasures.indexNames.length !== 0) {
          throw issue('unsupported-storage');
        }
        const run = () => queue(store, (value) => { check(); result = value; ready = true; }, abort, check, erasures);
        if (erasing) run();
        else {
          // Both lookups and the operation share one native transaction. A
          // queued writer cannot slip between the barrier check and its write.
          const global = erasures.get(ALL_OWNERS);
          const owner = erasures.get(ownerKey);
          let remaining = 2;
          const checked = () => {
            try {
              check();
              if (--remaining !== 0) return;
              const markers = [parseErasure(global.result, ALL_OWNERS), parseErasure(owner.result, ownerKey)];
              if (markers.some((marker) => marker?.status === 'pending')) throw issue('erasure-pending');
              if (markers.some((marker) => marker?.status === 'complete')) throw issue('owner-erased');
              run();
            } catch (error) { abort(error); }
          };
          global.onsuccess = checked; owner.onsuccess = checked;
          global.onerror = () => abort(global.error); owner.onerror = () => abort(owner.error);
        }
      } catch (error) { abort(error); }
    });
  }
  ...
  private requestErasure(target: string, guard: Guard): Promise<void> {
    return this.execute(target, 'readwrite', guard, (_store, done, fail, check, erasures) => {
      const request = erasures.get(target);
      request.onsuccess = () => {
        try {
          check();
          if (!parseErasure(request.result, target)) {
            const add = erasures.add({ target, status: 'pending' } satisfies Erasure);
            add.onerror = () => fail(add.error);
          }
          done(undefined);
        } catch (error) { fail(error); }
  ...
  private finishErasure(target: string): Promise<void> {
    return this.execute(target, 'readwrite', () => {}, (store, done, fail, check, erasures) => {
      const request = erasures.get(target);
      request.onsuccess = () => {
        try {
          check();
          const marker = parseErasure(request.result, target);
          if (!marker) throw issue('access-denied');
          if (marker.status === 'complete') { done(undefined); return; }
          const cursorRequest = target === ALL_OWNERS ? store.openKeyCursor() : store.index(OWNER_INDEX).openKeyCursor(target);
          cursorRequest.onsuccess = () => {
            try {
              ...
              if (!cursor) {
                ...
                // the prior durable pending marker and all original rows intact.
                const acknowledge = erasures.put({ target, status: 'complete' } satisfies Erasure);
                acknowledge.onerror = () => fail(acknowledge.error);
                done(undefined);
                return;
              }
  ...
  private async erase(target: string, guard: Guard): Promise<void> {
    let intentCommitted = false;
    try {
      await this.requestErasure(target, guard);
      intentCommitted = true;
      // Normal authority may now be revoked. Recovery cannot read receipts,
      // create new intent, or make a terminal owner writable again.
      await this.finishErasure(target);
    } catch (error) { throw rethrow(error, intentCommitted); }
  }

  clear(ownerKey: string, guard: Guard): Promise<void> {
    if (!isSavedNatalOwnerKey(ownerKey)) return Promise.reject(issue('invalid-input'));
    return this.erase(ownerKey, guard);
  }

  /** Application must authorize the entire device scope before requesting this. */
  async clearAll(guard: Guard): Promise<SavedNatalResult<void>> {
    try { await this.erase(ALL_OWNERS, guard); return { ok: true, value: undefined }; }
   ...
   * Completed barriers are retained. Readmission needs a future reviewed epoch
   * protocol; a fresh handle/reload is deliberately insufficient.
   */
  async recoverPendingErasures(): Promise<SavedNatalResult<number>> {
    let completed = 0;
    try {
      const targets = await this.execute<string[]>(ALL_OWNERS, 'readonly', () => {}, (_store, done, fail, check, erasures) => {
        // Bound recovery metadata; fail closed without partially parsing it.
        const request = erasures.getAll(undefined, 1001);
        request.onsuccess = () => {
          try {
            check();
            if (request.result.length > 1000) throw issue('unsupported-storage');
            const pending: string[] = [];
            for (const row of request.result) {
              const marker = parseErasure(row, row?.target);
              if (!marker) throw issue('unsupported-storage');
              if (marker.status === 'pending') pending.push(marker.target);
            }
            done(pending);
          } catch (error) { fail(error); }
        };
...
export class SavedNatalStore {
  ...
  /**
   * Explicit owner erasure consumes this handle, including on failure. The
   * native adapter durably fences all handles after intent commits. If intent
   * could not commit, this is NOT successful erasure; caller must remain locked
   * and retry with authority. Persisted intents permit erase-only recovery.
   * Application account/discovery integration remains inactive.
   */
