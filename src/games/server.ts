 ...
 */
import { SIGN_SLUGS } from '../lib/signs.js';
import {
  gamesQuestionForWeek,
  isGamesAnswer,
  type GamesWeeklyQuestion,
} from './questions.js';
import { daysLeftInSeason, isoWeekUtc, seasonInstanceAt } from './season.js';
import { gamesAnonymousPrincipal } from './security.js';

const MAX_BODY_BYTES = 256;
const MAX_RESPONSE_BYTES = 64 * 1024;
const DEFAULT_JOIN_BUCKET_LIMIT = 5;
...
export function parseCheckinAnswer(
  input: unknown,
  question: GamesWeeklyQuestion,
): string | null {
  const answer = parseBody(input)?.answer;
  return isGamesAnswer(question, answer) ? answer : null;
}
...
export interface GamesAnswerCount {
  key: string;
  count: number;
}

/** Preserve the reviewed answer order and never invent a response. */
export function zeroFilledAnswerCounts(
  rows: unknown,
  question: GamesWeeklyQuestion,
  signFilter?: string,
): GamesAnswerCount[] {
  ...
  if (Array.isArray(rows)) {
    for (const row of rows) {
      if (!row || typeof row !== 'object') continue;
      const { sign, answer_key: key, count } = row as Record<string, unknown>;
      if (typeof sign !== 'string' || !SIGN_SLUGS.includes(sign)) continue;
      if (signFilter && sign !== signFilter) continue;
      if (!isGamesAnswer(question, key)) continue;
      if (typeof count !== 'number' || !Number.isSafeInteger(count) || count < 0) continue;
      counts.set(key, (counts.get(key) ?? 0) + count);
    }
  }
  return question.choices.map(({ key }) => ({ key, count: counts.get(key) ?? 0 }));
}

export function zeroFilledTeamAnswerCounts(rows: unknown, question: GamesWeeklyQuestion) {
  return SIGN_SLUGS.map((sign) => ({
    sign,
    responses: zeroFilledAnswerCounts(rows, question, sign),
  }));
}
  ...
  try {
    if (action === 'standings') {
      ...
      sendJson(res, 200, {
        ...
        question: {
          ...question,
          responses: zeroFilledAnswerCounts(result?.responses, question),
          teams: zeroFilledTeamAnswerCounts(result?.responses, question),
        },
      }, req.query?.fresh !== '1');
      return;
    }
    ...
    if (action === 'join') {
      ...
      const principal = gamesAnonymousPrincipal(req, true, instant, deps.random);
      if (!principal) {
        sendJson(res, 503, { error: 'unconfigured' });
        return;
      }
    ...
    const principal = gamesAnonymousPrincipal(req, false, instant, deps.random);
    if (!principal) {
    ...
    const week = isoWeekUtc(instant);
    const question = gamesQuestionForWeek(week.isoYear, week.isoWeek);
    const answer = parseCheckinAnswer(req.body, question);
    if (!answer) {
      sendJson(res, 400, { error: 'invalid' });
      return;
    }
    ...
    sendJson(res, 200, {
      ...
      sign: result?.sign,
      seasonId: result?.season_id ?? season.id,
      isoYear: week.isoYear,
      isoWeek: week.isoWeek,
