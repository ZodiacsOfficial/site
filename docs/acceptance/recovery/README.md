...
The subsequent Site Check `33965690473` on `fbffd930b2440868b66b559a1645852a94918e0c` passed all 14 jobs, including actual PostgreSQL 17 migrations, replay and concurrency checks in job `101305302747`. That evidence applies to its captured stack; a newly integrated stack still needs its own final gate.

A downstream Moon full-suite run exposed a five-second timeout in the inherited never-ready test double. Its unchanged focused rerun passed in 3.257 seconds, so the full-suite failure remains recorded rather than treated as a passing gate. The never-ready fixture now uses a Bash-only Docker double and records each observed probe, SQL attempt, diagnostic and cleanup action. It avoids 60 Node startups while retaining all 60 real harness iterations, the original test and subprocess timeouts, and the fatal/no-SQL/no-success assertions. The other three lifecycle doubles and production harness are unchanged. Focused validation is separate from the required fresh full suite.
