...
for (let i = 0; i < BODIES.length; i += 1) {
  ...
  for (const tol of TOLERANCES) for (const relightWidthRatio of RATIOS) {
    const t0 = Date.now();
    let r;
    try {
      r = searchDeflectedOverPartition(eph, {
        body, targetDeg, fromTdbSec: from, toTdbSec: to, packDigest: dg,
        boundaryToleranceSec: tol, relightWidthRatio, maxEvaluations: 4_000_000, maxCells: 400_000,
      });
    } catch (e) {
      console.log(`${id} ${body} tol=${tol} r=${relightWidthRatio}: THREW ${e.code ?? ''} ${e.message.slice(0, 80)}`);
      continue;
    }
    const req = to - from;
    const row = {
      id, body, tol, relightWidthRatio,
    ...
    rows.push(row);
    console.log(`${id.padEnd(3)} ${body.padEnd(8)} tol=${String(tol).padStart(3)} r=${String(relightWidthRatio).padStart(2)} ${row.status.padEnd(16)} total=${String(row.total).padStart(9)} (part ${String(row.partition).padStart(9)} + search ${String(row.search).padStart(8)}) ratio=${row.ratio.toFixed(2)}x boundary=${row.boundaryPct.toFixed(3)}% excl=${row.excludedPct.toFixed(2)}% events=${row.events}/${row.established}`);
  }
}
