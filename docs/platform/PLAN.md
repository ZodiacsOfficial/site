...
| 9 | A04/C02 contract/receipts | Civil import/clock/timezone correction implemented: final 4,624 tests/408 files, 18 captures and 12 actual share journeys pass; draft #418 at a4284d8d and its exact READY preview are verified; All 14 jobs passed in exact-head CI 34161649806. Existing shape, year window, valid DST/LMT and saved-receipt fallback are preserved. Next: resolve requested/actual house semantics across save/rerun/export/sync with legacy-unknown and canonical-wire compatibility; the one-field remote normalization was disproved by a synthetic round trip. Then complete reviewed range/degenerate-angle policy and portable receipts. |
...
- GeoNames rejected transport/HTTP/JSON-parsing caches are corrected in SDK
  ...
  packed consumers passed. Structurally invalid fulfilled JSON still needs a
  schema/cache-policy decision. The site/starter retain immutable rc.1.
