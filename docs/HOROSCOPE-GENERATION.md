...
- Frontmatter: `sign`, `month: "YYYY-MM"`, `published: YYYY-MM-DD`,
  `updated: YYYY-MM-DD`, and `draft: true` while the previous month is still
  the live one — the horoscope routes render the latest NON-DRAFT month, so
  flip `draft` to `false` in a one-line PR on the 1st.
- If a lunation in the JSON is also an eclipse (check the month's almanac
  article or NASA), say so where the lunation is mentioned — the almanac and
  the horoscopes must not disagree about the same sky.
