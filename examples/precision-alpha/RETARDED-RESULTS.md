...
Generated by the preregistered rule and not touched. Matching is against
an **independent reference** written for this run (`reference.mjs`): the
...
Built **after** the holdout ran, and labelled so. The one defence against
fishing for a pass is that the target is not chosen: it is **read off the
body's own retarded longitude at the midpoint of the same preregistered
window**, rounded to 10⁻⁶ °, so there is no free parameter left to tune.
T1 replaces S1 the same way, on Mars's 2018 retrograde.
...
| | |
| --- | --- |
| \|τ_pack − τ_Swiss\|, max | **3.69 × 10⁻⁵ s** (Pluto, on 14 774 s) |
...
says to report it and not bound it, because it belongs to the compressed
pack and to two different source ephemerides. Reporting is not the same
...
| **difference — all the light-time implementation can contribute** | **+0.000068″** | — | **0.00129″** |
...
where the body moves quickly: 1.3 mas on the Moon is a τ difference of
...
The Sun is the instructive case: τ is 506 s, yet `NONE − LT/CN` is
0.008″. The correction retards the **Sun**, whose barycentric speed is
about 0.01 km/s — it does not retard the Earth, whose motion is what
makes the Sun appear to move a degree a day. Software that "adds
light-time" by shifting the observer instead gets ~21″ here, in the wrong
direction, which is the mistake `retarded.mjs` exists to avoid and L3/L4
exist to catch.
...
Peak heap over a search: **≤ 3.6 MB** above baseline, on every case.
...
both modes — same event counts, `validated-geometric` still `proven`
everywhere.
...
- **Proven means proven of the pack.** Completeness is established for the
  function the stored Chebyshev polynomial defines. The pack differs from
  Swiss's source by ~0.008″ systematically (§4b) and that is outside every
